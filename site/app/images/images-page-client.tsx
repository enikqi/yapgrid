...
export default function ImagesPageClient() {
  ...
  const hiddenPostsCacheRef = useRef<string[] | null>(null)
  const lastHiddenPostsCheckRef = useRef<number>(0)
  ...
  const fetchPosts = useCallback(async (pageNum: number) => {
    try {
      ...
      const params = new URLSearchParams({
        ...
        status: 'PUBLISHED',
        includeNsfw: 'false',
        assetType: 'THUMBNAIL' // Images only
      })

      // Add hidden post IDs from localStorage (cached for performance)
      const now = Date.now()
      if (!hiddenPostsCacheRef.current || (now - lastHiddenPostsCheckRef.current > 5000)) {
        try {
          hiddenPostsCacheRef.current = JSON.parse(localStorage.getItem('hiddenPosts') || '[]')
          lastHiddenPostsCheckRef.current = now
        } catch (e) {
          hiddenPostsCacheRef.current = []
        }
      }
      if (hiddenPostsCacheRef.current && hiddenPostsCacheRef.current.length > 0) {
        params.append('hiddenPostIds', hiddenPostsCacheRef.current.join(','))
      }
      ...
      if (data.success) {
        ...
        if (pageNum === 1) {
          ...
          // Update hasMore after state update
          // If we got no items or API says no more, stop loading
        ...
        // If API call failed, stop trying to load more
        setHasMore(false)
      }
    } catch (error) {
      console.error('Failed to fetch image posts:', error)
      setHasMore(false)
    } finally {
      setLoading(false)
  ...
  useEffect(() => {
    ...
    // Immediately check if sentinel is already in viewport (for cases where content is short)
    setTimeout(() => {
      const rect = sentinel.getBoundingClientRect()
      const viewportHeight = window.innerHeight || document.documentElement.clientHeight
      // Check if sentinel is within the extended viewport (including rootMargin)
      const isAlreadyVisible = rect.top < (viewportHeight + 100) && rect.bottom > -100
      if (isAlreadyVisible && !loadMoreLoading && hasMore && !loading) {
        loadMore()
      }
  ...
  }, [hasMore, loadMore, posts.length, loadMoreLoading, loading])
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        <div className="flex gap-6">
          ...
          <main className="flex-1 max-w-3xl">
            {/* Page Title */}
            <div className="mb-6">
              <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-2">Images</h1>
              <p className="text-gray-600 dark:text-gray-400">View images from YapGrid communities</p>
            </div>
              ...
              <>
                ...
                <div ref={loadMoreSentinelRef} className="h-20 flex items-center justify-center">
                  ...
                  {!hasMore && posts.length > 0 && (
                    <p className="text-gray-500 dark:text-gray-400">No more images to load</p>
                  )}
                </div>

                {posts.length === 0 && !loading && (
                  <div className="text-center py-12">
                    <p className="text-gray-500 dark:text-gray-400">No image posts found</p>
                  </div>
                )}
              </>
            )}
